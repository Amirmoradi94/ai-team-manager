...
class TaskHistoryManager {
  ...
  async loadState() {
    ...
    // Ensure mycompany/cto history mirrors current state
    try {
  ...
  async persistState() {
    ...
    lines.push('| taskId | title | type | provider | outcome | reason | timestamp | teamId | teamName |');
    lines.push('|--------|-------|------|----------|---------|--------|-----------|--------|----------|');

    for (const record of this.history.slice(-100)) {
      lines.push(
        `| ${record.taskId} | ${record.title} | ${record.type} | ${record.provider} | ${record.outcome} | ${record.reason} | ${record.timestamp} | ${record.teamId || ''} | ${record.teamName || ''} |`
      );
    }

    await fs.writeFile(this.historyFile, lines.join('\n'));
    try {
      const mycompanyDir = path.dirname(this.mycompanyHistoryFile);
      await fs.mkdir(mycompanyDir, { recursive: true });
      await fs.writeFile(this.mycompanyHistoryFile, lines.join('\n'));
    } catch (e) {
      // Non-fatal if mycompany directory is unavailable
    }
  }
