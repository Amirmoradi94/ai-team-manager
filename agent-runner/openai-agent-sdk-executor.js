...
class OpenAIAgentSdkExecutor {
  constructor(taskAPI) {
    ...
    this.projectRoot = path.resolve(__dirname, 'openai_agents');
  }
  ...
  async _runPython(payload, workingDir) {
    ...
    return new Promise((resolve, reject) => {
      ...
      child.on('close', (code) => {
        if (!stdout) {
          return reject(new Error(stderr || `OpenAI SDK runner failed (exit ${code})`));
        }
        try {
          ...
          reject(new Error(`Failed to parse OpenAI SDK output: ${err.message}. stderr=${stderr}`));
        }
      });
...
module.exports = OpenAIAgentSdkExecutor;
