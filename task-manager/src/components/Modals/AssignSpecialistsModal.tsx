...
interface AssignSpecialistsModalProps {
  team: any | null;
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
...
export function AssignSpecialistsModal({ team, isOpen, onClose, onSuccess }: AssignSpecialistsModalProps) {
  ...
  const fetchData = async () => {
    try {
      ...
      if (allRes.ok && assignedRes.ok) {
        ...
        setAssignedIds(currentTeam?.specialists?.map((s: any) => s.id) || []);
      }
    } catch (e) {
  ...
  const handleToggle = async (specId: string) => {
    ...
    // We update the team with its full set of specialist IDs
    ...
    try {
      const res = await fetch(`${API_URL}/teams/${team.id}`, {
        ...
        body: JSON.stringify({ specialist_ids: newIds })
      });
