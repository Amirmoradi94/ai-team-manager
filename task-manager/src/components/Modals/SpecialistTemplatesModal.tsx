...
import { SPECIALIST_TEMPLATES, CATEGORIES, SpecialistTemplate } from '@/data/specialistTemplates';

const API_URL = 'http://localhost:3001/api';

interface SpecialistTemplatesModalProps {
  isOpen: boolean;
  onClose: () => void;
...
export function SpecialistTemplatesModal({
  isOpen,
  onClose,
  onSelectTemplate,
  onCreateFromScratch
}: SpecialistTemplatesModalProps) {
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/ai/recommendations`, {
        ...
        body: JSON.stringify({
          goal,
          specialists: SPECIALIST_TEMPLATES
        })
      });
  ...
  const handleBulkAdd = async () => {
    ...
    if (successCount > 0) {
      ...
      onClose();
      // Trigger a page refresh or callback to reload specialists
  ...
  const filteredTemplates = SPECIALIST_TEMPLATES.filter(template => {
    // If we have recommendations, ONLY show recommended specialists
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
          ...
          <motion.div
            ...
            className="relative w-full max-w-5xl z-10 max-h-[90vh] flex flex-col"
            ...
            <div className="glass-card-dark p-6 glow-border flex flex-col max-h-[90vh]">
              ...
              <div className="flex items-center justify-between mb-6">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground mt-1">
                    Choose from {SPECIALIST_TEMPLATES.length} ready-to-use roles or create your own
                  </p>
                </div>
                <button
                  onClick={onClose}
              ...
              <div className="mb-6 p-3 rounded-lg bg-primary/5 border border-primary/20">
                ...
                <p className="text-xs text-muted-foreground mb-2">
                  Describe your goal and let AI suggest the optimal team composition
                </p>
                  ...
                  <div className="mt-3 p-3 rounded bg-secondary/30 space-y-3">
                    <div className="flex items-center justify-between">
                      <p className="text-xs font-medium text-foreground">
                        ✨ {recommendedIds.length} Roles Recommended
                      </p>
                      <button
                        onClick={() => {
                          setRecommendedIds([]);
                ...
                >
                  <div className="glass-card-dark p-6 glow-border flex flex-col max-h-[90vh]">
                    ...
                    <div className="flex items-center justify-between mb-6">
                      <div>
                        <h2 className="text-2xl font-semibold text-foreground flex items-center gap-2">
                          ...
                          Review Selected Roles ({selectedRoleIds.length})
                        </h2>
                        <p className="text-sm text-muted-foreground mt-1">
                          Review and customize roles before adding them to your team
                        </p>
                      </div>
                      <button
                    ...
                    <div className="flex gap-3 pt-4 border-t border-border">
                      ...
                      >
                        Add All {selectedRoleIds.length} Roles
                      </Button>
                    </div>
                  </div>
                </motion.div>
